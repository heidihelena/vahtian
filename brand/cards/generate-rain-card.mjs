// Generate the /rain/ social share card.
// Usage: node brand/cards/generate-rain-card.mjs
//
// The card is a real frame of the artwork, not a drawing of it: the script
// serves the repo, opens rain/ambient.html, drives its deterministic
...
const OUTPUT = join(HERE, "rain-share-card.jpg");
const require = createRequire(import.meta.url);
const { chromium } = require(
  join(ROOT, ".claude", "skills", "run-vahtian", "node_modules", "playwright"),
);

// How long the rain is allowed to fall before the shutter. The opening
// releases one new note every 0.30 s, so this needs to be past all 48 (about
// 14.4 s) for the card to show the full range rather than the bottom octaves.
const RUN_MS = 21000;
...
await page.goto(`${base}/rain/ambient.html`, { waitUntil: "load" });
await page.evaluate(() => document.fonts.ready);
await page.evaluate(() => document.getElementById("hud").remove());
await page.waitForTimeout(RUN_MS);        // let the rain reach the top of its range

await page.evaluate(({ ladder }) => {
  const card = document.createElement("div");
  card.innerHTML = `
    <div class="scrim"></div>
    <div class="type">
      <p class="kicker">digital rain</p>
      <p class="ask">forty-eight notes descending</p>
    </div>
    <p class="by">Heidi Anders&eacute;n &middot; vahtian.com/rain</p>
...
`;
  ...
  css.textContent = `
    ...
    .ask{margin:0;font-size:96px;line-height:1.06;font-weight:300;font-style:italic;
...
console.log(`✓ brand/cards/rain-share-card.jpg (${Math.round(bytes / 1024)} kB)`);
